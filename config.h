...
public:
    ...
    //set the scale of zoom objects should be set at
    void setScale(int scale) { m_scale = scale;}

    //get the scale of zoom objects should be set at
    int getScale() { return m_scale; }

    //get the y offset of planetary objets - used for moving around simulation
    int getXOffset() { return m_xOffset; }
    //offset operations
    void increaseXOffset() { m_xOffset += 30; }
    void decreaseXOffset() { m_xOffset -= 30; }
    void setXOffet(int offset) { m_xOffset = offset; }

    //get the x offset of planetary objets - used for moving around simulation
    int getYOffset() { return m_yOffset; }
    //offset operations
    void increaseYOffset() { m_yOffset += 30; }
    void decreaseYOffset() { m_yOffset -= 30; }
    void setYOffet(int offset) { m_yOffset = offset; }
