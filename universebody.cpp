...
void UniverseBody::render(QPainter& painter) const
{
    ...
    double x = ((m_xPosition / config->getDistanceScale()) * config->getScale()) + config->getXOffset() ;
    double y = ((m_yPosition / config->getDistanceScale()) * config->getScale()) + config->getYOffset();
    double radius = (m_radius / config->getRadiusScale() * config->getScale());

    if(config->getUseLogRadius())
    {
        radius = std::log(m_radius / config->getLogPointRadius()) * config->getScale();
    }

    if( (radius * config->getScale()) < 1 ) //this should also be zoomed?
    {
...
void UniverseBody::accept(Visitor* visitor)
{
    visitor->visit(this);
}
